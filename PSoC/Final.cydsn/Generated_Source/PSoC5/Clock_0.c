...
void Clock_0_Start(void) 
{
    ...
    Clock_0_CLKEN |= Clock_0_CLKEN_MASK;
	Clock_0_CLKSTBY |= Clock_0_CLKSTBY_MASK;
}
...
void Clock_0_Stop(void) 
{
    ...
    Clock_0_CLKEN &= (uint8)(~Clock_0_CLKEN_MASK);
	Clock_0_CLKSTBY &= (uint8)(~Clock_0_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_0_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_0_CLKEN & Clock_0_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_0_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_0_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Clock_0_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_0_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_0_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_0_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_0_GetSourceRegister();
    uint16 oldDivider = Clock_0_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_0_CLKEN & Clock_0_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_0_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(Clock_0_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_0_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_0_MOD_SRC |= modeBitMask & (uint8)Clock_0_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_0_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_0_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_0_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_0_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_0_MOD_SRC & (uint8)(Clock_0_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_0_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_0_GetDividerRegister();
    uint8 oldSrc = Clock_0_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_0_MOD_SRC |= CYCLK_SSS;
        Clock_0_MOD_SRC =
            (Clock_0_MOD_SRC & (uint8)(~Clock_0_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_0_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_0_MOD_SRC & Clock_0_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_0_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_0_PHASE = clkPhase & Clock_0_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_0_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_0_PHASE & Clock_0_PHASE_MASK;
}
